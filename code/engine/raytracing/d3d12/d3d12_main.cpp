...
void GL_InitRaytracing( int width, int height )
{
		...
		rsc.AddHeapRangesParameter(
			{
				...
				{ 0 /*b0*/, 1, 0, D3D12_DESCRIPTOR_RANGE_TYPE_CBV /*Camera parameters*/, 3 },
				{ 5 /*t5*/, 1, 0, D3D12_DESCRIPTOR_RANGE_TYPE_SRV /*megatexture*/, 7 },
			} );
	...
	Com_Printf( "Loading Raytracing Shaders...\n" );
	...
	Com_Printf( "Compiling Raytracing Pipeline...\n" );
	...
	Com_Printf( "Creating Raytrace Output Buffer...\n" );
	...
	// Camera matrix stuff.
	{
		...
		m_cameraBuffer = nv_helpers_dx12::CreateBuffer(
			m_device.Get(), m_cameraBufferSize, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, nv_helpers_dx12::kUploadHeapProps );
...
void GL_Init( HWND hwnd, HINSTANCE hinstance, int width, int height )
{
	...
	Com_Printf( "Creating D3D12 device...\n" );
	...
	Com_Printf( "Checking for raytracing support...\n" );
	...
	Com_Printf( "Creating GPU fences...\n" );
	...
	{
		{
			CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
				lightTexture->dx_resource, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
			m_commandList->ResourceBarrier( 1, &transition );
		}

		{
			CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
				albedoTexture->dx_resource, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
			m_commandList->ResourceBarrier( 1, &transition );
		}
		...
		{
			CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
				lightTexture->dx_resource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_SOURCE );
			m_commandList->ResourceBarrier( 1, &transition );
		}

		{
			CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
				albedoTexture->dx_resource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_SOURCE );
			m_commandList->ResourceBarrier( 1, &transition );
		}
		...
		CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
			compositeTexture->dx_resource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_COPY_SOURCE );
		m_commandList->ResourceBarrier( 1, &transition );
		...
		{
			CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
				compositeTexture->dx_resource, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
			m_commandList->ResourceBarrier( 1, &transition );
		}
	}
...
void GL_FinishDXRLoading( void )
{
	...
	m_device->CreateUnorderedAccessView( albedoTexture->dx_resource, nullptr, &uavDesc, srvHandle );

	// Add the Top Level AS SRV right after the raytracing output buffer
	srvHandle.ptr += m_device->GetDescriptorHandleIncrementSize(
		D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );

	m_device->CreateUnorderedAccessView( lightTexture->dx_resource, nullptr, &uavDesc, srvHandle );
	...
	srvHandle.ptr += m_device->GetDescriptorHandleIncrementSize( D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV );
	GL_LoadMegaNormalTexture( srvHandle );
...
void GL_Render( float x, float y, float z, vec3_t viewaxis[ 3 ] )
{
	...
	{
		CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
			albedoTexture->dx_resource, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
		m_commandList->ResourceBarrier( 1, &transition );
	}

	{
		CD3DX12_RESOURCE_BARRIER transition = CD3DX12_RESOURCE_BARRIER::Transition(
			lightTexture->dx_resource, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
		m_commandList->ResourceBarrier( 1, &transition );
	}

	if( raytracingDataInit == 0 )
	{
		return;
	}
	...
	create_view_matrix( ( float* )&matrices[ 0 ], vieworg, viewaxis );

	//float m_aspectRatio = glConfig.vidWidth / glConfig.vidHeight;
